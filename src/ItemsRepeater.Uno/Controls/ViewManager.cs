...
using Control = Microsoft.UI.Xaml.FrameworkElement;
    ...
    internal sealed class ViewManager
    {
        ...
        Control? FindFocusCandidate(int clearedIndex, out Control? focusedChild)
        {
            ...
            foreach (var child in _owner.Children)
            {
                ...
                var virtInfo = ItemsRepeater.TryGetVirtualizationInfo(childElement);
                ...
                case NotifyCollectionChangedAction.Add:
                    {
                        ...
                        if (newIndex <= _lastRealizedElementIndexHeldByLayout)
                        {
                            ...
                            {
                                if (element is not Control realizedElement)
                                {
                                    continue;
                                }

                                var virtInfo = ItemsRepeater.GetVirtualizationInfo(realizedElement);
                                var dataIndex = virtInfo.Index;

                                if (virtInfo.IsRealized && dataIndex >= newIndex)
                                {
                                    UpdateElementIndex(realizedElement, virtInfo, dataIndex + newCount);
                                }
                            }
                        }
                        else
                        ...
                        if (countChange != 0)
                        {
                            ...
                            foreach (var element in _owner.Children)
                            {
                                ...
                                if (virtInfo.IsRealized)
                                {
                                    ...
                                    {
                                        UpdateElementIndex(realizedElement, virtInfo, dataIndex + countChange);
                                    }
                                }
                            }
                        ...
                        foreach (var element in _owner.Children)
                        {
                            ...
                            if (virtInfo.IsRealized)
                            {
                                if (virtInfo.AutoRecycleCandidate && oldStartIndex <= dataIndex && dataIndex < oldStartIndex + oldCount)
                                {
                                    ...
                                    _owner.ClearElementImpl(realizedElement);
                                }
                                else if (dataIndex >= (oldStartIndex + oldCount))
                                {
                                    UpdateElementIndex(realizedElement, virtInfo, dataIndex - oldCount);
                                }
                            }
                        }
        ...
        private Control? GetElementIfAlreadyHeldByLayout(int index)
        {
            ...
            if (cachedFirstLastIndicesInvalid || isRequestedIndexInRealizedRange)
            {
                foreach (var child in _owner.Children)
                {
                    if (child is not Control realizedChild)
                    {
                        continue;
                    }

                    var virtInfo = ItemsRepeater.TryGetVirtualizationInfo(realizedChild);
                    if (virtInfo?.IsHeldByLayout == true)
                    {
        ...
        private Control GetElementFromElementFactory(int index)
        {
            ...
            Control GetElement()
            {
                ...
                try
                {
                    ...
                    args.Parent = _owner;
                    args.Index = index;
                    return elementFactory.GetElement(args) as Control
                        ?? throw new InvalidOperationException("Element factory must return a FrameworkElement.");
                }
                finally
                {
                    args.Data = null;
        ...
        private void UpdateFocusedElement()
        {
            ...
            if (_owner.XamlRoot is not null && FocusManager.GetFocusedElement(_owner.XamlRoot) is DependencyObject child)
            {
